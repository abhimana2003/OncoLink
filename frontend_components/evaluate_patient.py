...
def render(raw_df):
    _render_header()
    current_eval = _render_new_patient_intake()
    if not current_eval:
        st.info("Enter a new patient profile or select an external genomic case to begin evaluation.")
        return
    ...
    _render_similar_patients(raw_df, profile, feature_bundle, evaluation_id, summary)
...
def _render_new_patient_intake():
    ...
    if model is None:
        ...
        return st.session_state.get("current_evaluation")
...
def _render_explanation(profile,prediction,feature_bundle,evaluation_id,):
